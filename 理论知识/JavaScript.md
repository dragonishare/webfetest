...
> a.403 Forbidden 客户端错误，服务器已经理解请求，但是拒绝执行它。通常用于服务器上文件或目录的权限设置所致
...
### 16.js中的函数split(),join()
* split() 方法用于把一个字符串分割成字符串数组,并返回分割成的一个字符串数组;
  splite()，接受两个参数，第一个必需，进行分割的字符串；第二个可选，数组的大小
* join() 方法用于把数组中的所有元素放入一个字符串。通过指定的分隔符进行分隔的;
  join() 接受一个参数，可选，指定要使用的分隔符
